...
describe("Tree", () => {
	test("Tree is rendered correctly", async () => {
			...
			props: {
				...
				getItemProps(props) {
					return {
						...
						"data-parent-id": props.parent?.node.id,
		...
		async function expectTreeItemAttributes(
			...
			await expect.element(item).toHaveAttribute("aria-selected", String(attributes.ariaSelected));
			...
			await expect.element(item).toHaveAttribute("aria-posinset", String(attributes.ariaPosInSet));
			...
			if (attributes.parent === undefined) {
				await expect.element(item).not.toHaveAttribute("data-parent-id");
				await expect.element(item).not.toHaveAttribute("data-parent-index");
			} else {
				await expect.element(item).toHaveAttribute("data-parent-id", attributes.parent.id);
				await expect
					.element(item)
					.toHaveAttribute("data-parent-index", String(attributes.parent.index));
			}
	...
	describe("Delete", () => {
		test("Delete removes all selected items", async () => {
			const tree = createTree({
				...
				defaultCopied: ["1", "1.1", "1.1.1", "1.1.2", "1.1.3", "1.2", "1.2.1", "1.2.2"],
		...
		test("Delete does not delete the current item if it is not selected", async () => {
			...
			await commands.press("Delete");
			await expect.element(item_2).toBeVisible();
			expect(tree.nodes).toEqual([node_1, node_2, node_3]);
		});
